...
import { Trash2, Eye, Clock, ChevronLeft, ChevronRight, Filter, Pencil } from "lucide-react"
...
const estadoBadgeVariant = (estado: EstadoOrden) => {
  switch (estado) {
    case EstadoOrden.PENDIENTE: return "secondary"
    case EstadoOrden.PROCESO: return "default"
    case EstadoOrden.TERMINADO: return "success"
    case EstadoOrden.CANCELADO: return "destructive"
    ...
    <div className="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden border border-border/70">
      <div className="flex flex-col sm:flex-row gap-4 p-4 border-b border-border/70">
        ...
        <div className="w-full sm:w-auto sm:min-w-[180px]">
          <Select value={filterStatus} onValueChange={(value: EstadoOrden | "ALL") => { setFilterStatus(value); setCurrentPage(1); }}>
            ...
            <SelectContent>
              ...
              {Object.values(EstadoOrden).map((estado) => (
                <SelectItem key={estado} value={estado}>{getEtiquetaEstado(estado)}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-50 dark:hover:bg-gray-900/60 transition-colors">
              <TableHead className="cursor-pointer" onClick={() => handleSort("ordenNumero")}>N° Orden{getSortIcon("ordenNumero")}</TableHead>
              <TableHead className="cursor-pointer" onClick={() => handleSort("cliente.nombreRazonSocial")}>Cliente{getSortIcon("cliente.nombreRazonSocial")}</TableHead>
              <TableHead className="text-right cursor-pointer" onClick={() => handleSort("totalUSD")}>Total (USD){getSortIcon("totalUSD")}</TableHead>
              <TableHead className="text-center cursor-pointer" onClick={() => handleSort("estado")}>Estado{getSortIcon("estado")}</TableHead>
              <TableHead className="text-center cursor-pointer" onClick={() => handleSort("fecha")}>Fecha{getSortIcon("fecha")}</TableHead>
          ...
          <TableBody>
            {paginatedOrdenes.length > 0 ? (
              paginatedOrdenes.map((orden, index) => {
                ...
                // 🔥 CORRECCIÓN: Sanitización del número de orden
                const numeroOrdenDisplay = isNaN(Number(orden.ordenNumero)) ? "N/A" : orden.ordenNumero;

                return (
                  <TableRow key={orden.id} className={`${index % 2 === 0 ? "bg-white dark:bg-gray-800" : "bg-gray-50 dark:bg-gray-800/70"} hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors`}>
                    
                    {/* 🛡️ Renderizado Seguro del Número */}
                    <TableCell className="font-medium text-primary">
                        {numeroOrdenDisplay}
                    </TableCell>
                    
                    <TableCell>
                      <p className="font-medium leading-tight dark:text-gray-200">{orden.cliente?.nombreRazonSocial || "Sin cliente"}</p>
                      <p className="text-xs text-muted-foreground dark:text-gray-400">RIF: {orden.cliente?.rifCedula || "N/A"}</p>
                    </TableCell>
                    <TableCell className="text-right font-bold">{formatCurrency(orden.totalUSD)}</TableCell>
                    <TableCell className="text-center">
                      <Badge variant={estadoBadgeVariant(orden.estado || EstadoOrden.PENDIENTE)}>{getEtiquetaEstado(orden.estado)}</Badge>
                    </TableCell>
                    <TableCell className="text-center text-sm">
                      <p className={`font-semibold ${isDeliveryDate ? "text-green-600 dark:text-green-400" : "text-foreground dark:text-gray-200"}`}>{primaryDate}</p>
                      <p className="text-xs text-muted-foreground dark:text-gray-400 mt-0.5" title="Fecha de creación">Creada: {formatDate(orden.fecha)}</p>
                    </TableCell>
                    <TableCell className="text-center space-x-1">
                      <Button size="icon" variant="ghost" onClick={() => handleOpenDetailModal(orden)} title="Ver Detalles"><Eye className="w-4 h-4 text-blue-500" /></Button>
                      <Button size="icon" variant="ghost" onClick={() => onEdit(orden)} title="Editar Orden"><Pencil className="w-4 h-4 text-orange-500 hover:text-orange-600" /></Button>
                      <Button size="icon" variant="ghost" onClick={() => handleOpenStatusModal(orden)} title="Cambiar Estado"><Clock className="w-4 h-4 text-green-600 dark:text-green-500" /></Button>
                      <Button size="icon" variant="ghost" onClick={() => onDelete(orden.id)} title="Eliminar Orden"><Trash2 className="w-4 h-4 text-red-600 dark:text-red-500" /></Button>
                    </TableCell>
                  </TableRow>
                )
              })
            ) : (
              <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-6 dark:text-gray-400">No se encontraron órdenes.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
      ...
      <div className="flex items-center justify-between p-3 border-t border-border/70 bg-gray-50 dark:bg-gray-900/50 rounded-b-lg transition-colors">
        <Button variant="outline" size="sm" onClick={() => setCurrentPage((prev) => Math.max(1, prev - 1))} disabled={currentPage === 1}><ChevronLeft className="w-4 h-4 mr-1" /> Anterior</Button>
        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Página {currentPage} de {totalPages}</span>
        <Button variant="outline" size="sm" onClick={() => setCurrentPage((prev) => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages || totalPages === 0}>Siguiente <ChevronRight className="w-4 h-4 ml-1" /></Button>
      </div>
