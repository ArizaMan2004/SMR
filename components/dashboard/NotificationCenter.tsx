...
interface NotificationCenterProps {
    ...
    onMaximize: () => void // Función para llevar al centro de actividades
...
export function NotificationCenter({ 
    ...
    // Lógica para generar detalles con IA (SMR Intel)
    ...
    // Clasificación de notificaciones por fecha
        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    <motion.aside initial={{ x: 400, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 400, opacity: 0 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed top-0 right-0 h-full w-full md:w-[450px] z-[70] p-4 md:p-6">
                        <Card className="h-full rounded-[3rem] border-none shadow-2xl bg-white/95 dark:bg-[#1c1c1e]/95 backdrop-blur-2xl flex flex-col overflow-hidden">
                            <div className="p-8 border-b border-black/5 dark:border-white/5 space-y-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {/* CLAVE: Solo llama a onMaximize, el Dashboard se encarga del resto */}
                                    <Button 
                                        variant="ghost" 
                                        onClick={onMaximize} 
                                        className="rounded-xl font-bold text-[10px] uppercase gap-2 border border-black/5"
                                    >
                                        <LayoutDashboard className="w-3.5 h-3.5" /> Ver Centro
                                    </Button>
                                </div>
                            </div>
...
// --- COMPONENTES AUXILIARES (SECTION & CARD) ---
...
function NotificationCard({ note, isExpanded, aiDetail, onExpand, onDelete, onToggleRead }: any) {
    ...
    return (
        <motion.div layout className={cn("p-5 rounded-[2.5rem] border transition-all duration-300 relative", note.isRead ? "bg-black/5 border-transparent opacity-60" : "bg-white dark:bg-white/5 border-black/5 dark:border-white/5 shadow-sm")}>
            <div className="flex gap-4 cursor-pointer" onClick={onExpand}>
                <div className={cn(
                    ...
                    {React.isValidElement(note.icon) ? React.cloneElement(note.icon as React.ReactElement, { className: "w-5 h-5 text-white" }) : note.icon}
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                        ...
                        <span className="text-[9px] font-black text-slate-400 bg-white/50 dark:bg-slate-800 px-2 py-0.5 rounded-full whitespace-nowrap uppercase shadow-sm">{formatSMRDate(note.timestamp)}</span>
                    </div>
