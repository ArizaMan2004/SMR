...
export function NotificationCenter({ 
    ...
    const generateAIDetail = (note: Notification) => {
        const { metadata } = note;
        if (note.type === 'warning') return `SMR Intel: Tasa actualizada a Bs. ${metadata?.tasaNueva || 'N/A'}.`;
        if (note.type === 'success') return `Tesorería: Actividad financiera procesada correctamente.`;
        return note.description || `SMR Intel: Actividad procesada correctamente.`;
    }
                            ...
                            <div className="p-8 border-b border-black/5 dark:border-white/5 space-y-4">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="font-black text-2xl tracking-tighter italic uppercase leading-none text-slate-900 dark:text-white">Actividades</h3>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Button variant="ghost" onClick={onMaximize} className="rounded-xl font-bold text-[10px] uppercase gap-2 border border-black/5"><LayoutDashboard className="w-3.5 h-3.5" /> Ver Centro</Button>
                                </div>
                            </div>
                            <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                                <LayoutGroup>
                                    <NotificationSection title="Hoy" items={recientes} icon={<Sparkles className="w-3 h-3 text-blue-500" />} expandedId={expandedId} onExpand={setExpandedId} aiNarrative={generateAIDetail} onDelete={onDeleteNotification} onToggleRead={onToggleRead} />
                                ...
                                {activeNotifications.length === 0 && (
                                    <div className="h-full flex flex-col items-center justify-center text-center py-20 opacity-20"><CheckCircle2 className="w-20 h-20 mb-4" /><p className="font-black uppercase tracking-widest text-xs">SMR Limpio</p></div>
                                )}
                            </div>
                        </Card>
                    </motion.aside>
...
function NotificationCard({ note, isExpanded, aiDetail, onExpand, onDelete, onToggleRead }: any) {
    const formatSMRDate = (date: any) => {
        // Manejo robusto de fechas (Date o Firebase Timestamp)
        const d = date instanceof Date ? date : date?.toDate ? date.toDate() : new Date();
        const timeStr = d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
        return `${d.toLocaleDateString('es-VE')} — ${timeStr}`;
    };
        ...
        <motion.div layout className={cn("p-5 rounded-[2.5rem] border transition-all duration-300 relative", note.isRead ? "bg-black/5 border-transparent opacity-60 dark:bg-white/5" : "bg-white dark:bg-white/10 border-black/5 dark:border-white/5 shadow-sm")}>
            <div className="flex gap-4 cursor-pointer" onClick={onExpand}>
                <div className={cn(
                    ...
                    {React.isValidElement(note.icon) ? React.cloneElement(note.icon as React.ReactElement, { className: "w-5 h-5 text-white" }) : <Bell className="w-5 h-5 text-white" />}
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-1">
                        ...
                        <span className="text-[9px] font-black text-slate-400 bg-white/50 dark:bg-slate-800 px-2 py-0.5 rounded-full whitespace-nowrap uppercase">{formatSMRDate(note.timestamp)}</span>
                    </div>
            ...
            <AnimatePresence>{isExpanded && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-5 pt-5 border-t border-black/5 dark:border-white/5 space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                        <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); onToggleRead(); }} className="h-9 px-4 rounded-xl font-bold text-[10px] uppercase bg-black/5">{note.isRead ? "Marcar como no leído" : "Marcar como leído"}</Button>
